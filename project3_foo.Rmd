...
tidy_dataset <- dataset %>%
  filter(redcap_event_name == "event_1_arm_1") %>%      # TODO: filtered out other events on 12/8
...
```
\textbf{Model 2 (Age & Education Removed): Anova (type 2) test}
...
* The following points are then removed (.rownames, .cooksd): (63, 0.098), (3, 126.01), (66, 0.075), and (199, 0.046)
...
# Education
plt_vol_education <- ggplot(volume_dataset %>% select(Pic, Education) %>% drop_na(), aes(Education, Pic)) +
  geom_boxplot() +
  geom_jitter(height = 0) +
  labs(
...
# Age with Education
plt_vol_age_edu <- ggplot(volume_dataset %>% select(Age, Pic, Education) %>% drop_na(), aes(Age, Pic, color = Education)) +
  geom_jitter() +
  geom_smooth(method = lm , formula = y ~ x, se = FALSE) +
  labs(
...
```{r Cooks plots, remove one point at a time}
...
## Picture Sequence Memory Test Planned Analysis (Hypothesis 2): Cook's Distance by point removal and model comparison
```{r Cooks plots, remove one point at a time}
# TODO: CHRIS
