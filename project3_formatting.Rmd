...
invis_hypo1_model_2 <- drop1(hypo1_model_1)
...
```{r}
invis_hypo1_model_3 <- drop1(hypo1_model_2)
...
invis_hypo1_model_4 <- drop1(hypo1_model_3)
...
```{r}
#We use the drop1 function to iteratively produce a more parsimonious model based on comparing AIC. For concision, only the final drop1 output is printed, showing that removed any additional terms would not improve the model, as indicated by AIC comparison
invis_hypo2_model_dropped1 <- drop1(hypo2_model)
#Dropping Musical_Practice gives best improvement to AIC, therefore dropped
...
invis_hypo2_model_dropped2 <- drop1(hypo2_model_dropped1)
...
invis_hypo2_model_dropped3 <- drop1(hypo2_model_dropped2)
...
#We use the drop1 function to iteratively produce a more parsimonious model based on comparing AIC. For concision, only the final drop1 output is printed, showing that removed any additional terms would not improve the model, as indicated by AIC comparison
invis_Moca_unplanned_ageintmod_dropped1 <- drop1(Moca_unplanned_ageintmod)
...
```
\emph{Figure (Cooks' Distance): Using the general rules of thumb for highlighting outliers of > 3 * (average Cooks Distance) for Cooks
Distance value as well as > 2 * (average hat value) for .hat value.}
...
```
\emph{Figure (Cook's Distance): Using the general rules of thumb for highlighting outliers of > 3 * (average Cook's Distance) for Cooks
Distance value as well as > 2 * (average hat value) for .hat value. The lines represent the first three multiples of average .hat values}

```{r}
plt_vol_cooksd_std
```
\emph{Figure (Cook's Distance): Using the general rules of thumb for highlighting outliers of > 3 * (average Cook's Distance) for Cooks
Distance value as well as > 2 * (average hat value) for .hat value. The line represents the critical Cook's Distance}
...
Supplemental plots from the PHC volume and memory section are here.
