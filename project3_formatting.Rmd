...
ggplot(drop_na(hypo_one_dataset,Second_Language,Flanker_test), aes(x = Second_Language, y = Flanker_test,
  ...
  stat_summary(fun.data = mean_se, color = "purple") +
  geom_dotplot(binaxis = "y",stackdir = "center", binwidth = 0.8, show.legend = FALSE) +
...
ggplot(drop_na(hypo_one_dataset,Track_Activity,Flanker_test), aes(x = Track_Activity, y = Flanker_test,
  ...
  geom_boxplot(show.legend = FALSE) +
  geom_jitter(height = 0, alpha = 0.6, show.legend = FALSE)+
  stat_summary(fun = "mean", geom = "point", color = "black") +
...
kable(tidy(Anova(hypo1_model_1, type = 2, alpha = 0.05)), digits = 3,caption = "Anova for Hypothesis 1 Model 1")
kable(tidy(hypo1_model_1), digits = 3,caption = "T-test for Hypothesis 1 Model 1")
```
...
kable(tidy(Anova(hypo1_model_2, type = 2, alpha = 0.05)), digits = 3,caption = "Anova for Hypothesis 1 Model 2")
kable(tidy(hypo1_model_2), digits = 3,caption = "T-test for Hypothesis 1 Model 2")
...
#Anova of new model
kable(tidy(Anova(hypo1_model_3, type = 2, alpha = 0.05)), digits = 3,caption = "Anova for Hypothesis 1 Model 3")
kable(tidy(hypo1_model_3), digits = 3, caption = "T-test for Hypothesis 1 Model 3")
```
...
hypo1_model_3_aug %>% 
  ...
  geom_text(aes(label=if_else((.cooksd>=3*mean(.cooksd) & .hat>= 2*hat_avg_model_3), paste0("(", .rownames, ", ", round(.cooksd, digits = 3),")"),'')),  nudge_y = 0.3, nudge_x = 0.04, size=3) +
...
kable(tidy(Anova(new_hypo1_model, type = 2, alpha = 0.05)), digits = 3,caption = "Anova for model with removed outliers")
kable(tidy(new_hypo1_model),caption = "T-test for model with removed outliers")
...
ggplot(drop_na(hypo_two_dataset,Remembering,Pic_Seq_Mem_Test), aes(x = Remembering, y = Pic_Seq_Mem_Test,
  ...
  xlab("Have you been able to remember things as easily as usual?") +
  ylab("Picture Sequence Memory Test Score") +
...
kable(tidy(Anova(hypo2_model, type = 2, alpha = 0.05)), digits = 3,caption = "Anova for Hypothesis 2 Model 1")
kable(tidy(hypo2_model), digits = 3,caption = "T-test for Hypothesis 2 Model 1")
```
...
kable(tidy(Anova(hypo2_model_dropped3, type = 2, alpha = 0.05)), digits = 3,caption ="Anova for Hypothesis 2 Outliers Removed")
kable(tidy(hypo2_model_dropped3), digits = 3,caption ="T-test for Hypothesis 2 Outliers Removed")
```
...
ggplot(drop_na(exp_dataset,Concentration, Moca_Score), aes(Concentration,Moca_Score,color = Concentration))+
  ...
  geom_jitter(alpha=0.5, height = 0, show.legend = FALSE)+
  stat_summary(fun.data = mean_se, color = "black") +
  xlab("Ability to Concentrate") +
  ylab("MoCA Score") +
  ggtitle("MoCA Scores vs Ability to Concentrate")+
  theme_bw()+
  Project3_theme()
```
...
kable(tidy(Mocamod), caption = "T-test for Exploratory Question 1")
```
...
kable(Anova(Moca_unplanned_agemod),caption = "Anova for New Moca Model\n Age Included")
kable(tidy(Moca_unplanned_agemod),caption = "T-test for New Moca Model\n Age Included")
...
kable(Anova(Moca_unplanned_ageintmod),caption = "Anova for New Moca Model\n Age and Interactions Included")
kable(tidy(Moca_unplanned_ageintmod,caption = "T-test for New Moca Model\n Age and Interactions Included"))
...
kable(AIC(Moca_unplanned_mod,Moca_unplanned_agemod,Moca_unplanned_ageintmod), caption = "Comparing Models with Different Factors")
...
kable(drop1(Moca_unplanned_ageintmod_dropped1),caption = "Test if factor needs to be dropped")


kable(AIC(Moca_unplanned_ageintmod_dropped1,Moca_unplanned_mod),caption = "Compare Original Model with\n Drop1 Model")
```
...
Moca_newremove_mod <- lm(newMoca_unplanned,
                         formula = Moca_Score ~ Flanker_test + Pic_Seq_Mem_Test)
kable(Anova(Moca_newremove_mod),caption = "Anova for Moca model:\n Outliers Removed")
kable(tidy(Moca_newremove_mod),caption = "T-test for Moca model:\n Outliers Removed")
```
...
plt_vol_age <- ggplot(drop_na(volume_dataset,Age,Pic), aes(Age, Pic)) +
  ...
  labs(
    title = 'Age vs Picture Sequence Memory Score',
...
plt_vol_phc <- ggplot(drop_na(volume_dataset,Tot_PHC_normed,Pic), aes(Tot_PHC_normed, Pic)) +
  ...
  geom_smooth(method = lm , formula = y ~ x, se = FALSE) +
  labs(
    title = 'Picture Sequence Memory Score vs Parahippocampal Volume',
    y = 'Picture Sequence Memory Test',
    x = 'Parahippocampal Volume'
  )+
  theme_bw()+
  Project3_theme()
...
# Second Language
plt_vol_sec_lang <- ggplot(drop_na(volume_dataset,Sec_Lang,Pic), aes(Sec_Lang, Pic, color=Sec_Lang)) +
  geom_boxplot(show.legend = FALSE, outlier.shape = NA) +
  geom_jitter(alpha=0.5, height = 0, show.legend = FALSE) +
  stat_summary(fun.data = mean_se, color = "black") +
  labs(
    title = 'Picture Sequence Memory Score vs Second Language',
    y = 'Picture Sequence Memory Test',
    x = 'Second Language'
  )+
  theme_bw()+
...
# Education
plt_vol_education <- ggplot(drop_na(volume_dataset,Education,Pic), aes(Education, Pic, color = Education)) +
  geom_boxplot(show.legend = FALSE, outlier.shape = NA) +
  geom_jitter(alpha=0.5,height = 0,show.legend = FALSE) +
  stat_summary(fun.data = mean_se, color = "black") +
  labs(
    title = 'Picture Sequence Memory Score vs Education Level',
    y = 'Picture Sequence Memory Test'
  )+
  theme_bw()+
  Project3_theme()+
...
plt_vol_age_edu <- ggplot(drop_na(volume_dataset,Age,Pic,Education), aes(Age, Pic, color = Education)) +
  ...
  geom_smooth(method = lm , formula = y ~ x, se = FALSE) +
  labs(
    title = 'Picture Sequence Memory Score vs Age & Education',
    y = 'Picture Sequence Memory Test'
  )+
  theme_bw()+
  Project3_theme()
...
tbl_vol_full_mod_no_ints <- kable(tidy(Anova(full_vol_model_no_int)),
                                  caption = 'Anova for PSM Test Model \nwith No Interactions')
...
```{r, New Cook\'s plots for supplement}
#Remove First Point
