...
```
\emph{Table Interpretations: The second table that shows an Anova test of the model, only age contributes significantly to the sum of squares regression. A t-test on the model shows that only the age coefficient is significant and we can conclude that as age increases the unadjusted Flanker's test score of Participants will decrease. Therefore, the hypothesis is false that musical practice and the ability to speak a second language will have a signficiant effect on increasing Flanker's test scores.}
...
tbl_vol_comp_models <- kable(tibble(Model = c('Main effects only','Main effects and second order interactions of significant main effects','All effects', 'Main effects and Age-PHC-Education interaction', 'Age, PHC, Second Language, and their second order interactions'),
       R_Squared = c(glance(full_vol_model_no_int_out)$r.squared,glance(vol_int_1)$r.squared,glance(vol_int_2)$r.squared,glance(vol_int_3)$r.squared,glance(vol_int_4)$r.squared),
       AIC = c(glance(full_vol_model_no_int_out)$AIC,glance(vol_int_1)$AIC,glance(vol_int_2)$AIC,glance(vol_int_3)$AIC,glance(vol_int_4)$AIC)),
...
### Flanker's Test Planned Analysis (Hypothesis 1): Cook's distance by point removal and model comparison
...
removed_model_one <- lm(data = remove_first, Flanker_test ~ Age + Education_Level)

remove_second <- hypo1_model_3_aug %>%
  filter(.rownames != 199) %>%
  select(Flanker_test, Age, Education_Level)

removed_model_two <- lm(data = remove_second, Flanker_test ~ Age + Education_Level)

remove_third <- hypo1_model_3_aug %>% 
  filter(.rownames != 229) %>%
  select(Flanker_test, Age, Education_Level)

removed_model_three <- lm(data = remove_third, Flanker_test ~ Age + Education_Level)

remove_four <- hypo1_model_3_aug %>% 
  filter(.rownames != 252) %>%
  select(Flanker_test, Age, Education_Level)

removed_model_four <- lm(data = remove_four, Flanker_test ~ Age + Education_Level)

kable(AIC(removed_model_one,removed_model_two, removed_model_three, removed_model_four))
...
Flanker_fourthaug <- augment(removed_model_four)
```

```{r New cooks plots for supplement}
#Remove First Point
row_num_removed1 <- nrow(Flanker_firstaug)
avg_hatFlanker1 = (1+2)/row_num_removed1
...
Flanker_firstaug %>%
  ...
  labs(title = "Cooks Distance for Row Name 134")+
  theme_bw()+
  Project3_theme()

#Remove Second Point
row_num_removed2 <- nrow(Flanker_secondaug)
avg_hatFlanker2 = (1+2)/row_num_removed2
...
Flanker_secondaug %>%
  ...
  labs(title = "Cooks Distance for Row Name 199")+
  theme_bw()+
  Project3_theme()

#Remove Third Point
row_num_removed3 <- nrow(Flanker_thirdaug)
avg_hatFlanker3 = (1+2)/row_num_removed3
...
Flanker_thirdaug %>%
  ...
  labs(title = "Cooks Distance for Row Name 229")+
  theme_bw()+
  Project3_theme()

#Remove Fourth Point
row_num_removed4 <- nrow(Flanker_fourthaug)
avg_hatFlanker4 = (1+2)/row_num_removed4

Flanker_fourthaug %>%
  ggplot( aes(.hat, .std.resid, size=.cooksd))+
  geom_point(shape = 1)+
  geom_vline(xintercept = avg_hatFlanker4, linetype = 2)+
  geom_vline(xintercept = 2 * avg_hatFlanker4, linetype = 2, color="orange")+
  geom_vline(xintercept = 3 * avg_hatFlanker4, linetype = 2, color="red")+
  geom_hline(yintercept = c(-2, 2), linetype = 2)+
  geom_text(aes(label=if_else((.cooksd>=3*mean(.cooksd) & .hat>= 2*avg_hatFlanker4),  paste0("(", Flanker_test, ", ",")"),'')),  nudge_y = 0.2, size=3)+
  labs(title = "Cooks Distance for Row Name 252")+
  theme_bw()+
  Project3_theme()
```
