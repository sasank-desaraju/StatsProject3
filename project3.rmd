...
exp_dataset <- tidy_dataset %>%
  ...
  mutate(Total_PHC_normed = (R_Parahippo_vol + L_Parahippo_vol) / Est_Tot_intra_cranial_vol) %>%
  mutate(Second_Language = replace_na(Second_Language, 0)) %>%  #assigns 0 to subjects who don't speak a second language
  mutate(Musical_Practice = replace_na(Musical_Practice, 0)) %>%  #assigns 0 to subjects who don't practice an instrument
...
ggplot(exp_dataset, aes(Age, Pic_Seq_Mem_Test)) +
  geom_jitter() +
  geom_smooth(method = lm , formula = y ~ x, se = FALSE)

ggplot(exp_dataset, aes(Musical_Practice, Pic_Seq_Mem_Test)) +
  geom_jitter() +
  geom_smooth(method = lm , formula = y ~ x, se = FALSE)

ggplot(exp_dataset, aes(Total_PHC_normed, Pic_Seq_Mem_Test)) +
  geom_jitter() +
  geom_smooth(method = lm , formula = y ~ x, se = FALSE)

ggplot(exp_dataset, aes(Second_Language, Pic_Seq_Mem_Test)) +
  geom_jitter() +
  geom_smooth(method = lm , formula = y ~ x, se = FALSE)

ggplot(exp_dataset, aes(Education_Level, Pic_Seq_Mem_Test)) +
  geom_jitter() +
  geom_smooth(method = lm , formula = y ~ x, se = FALSE)
...
full_pic_model_no_int <- lm(exp_dataset, formula = Pic_Seq_Mem_Test ~ Age + Musical_Practice + Total_PHC_normed + Second_Language + Education_Level)
